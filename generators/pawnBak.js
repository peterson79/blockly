...
Blockly.Pawn.addReservedWords(
    'assert,break,case,catch,class,const,continue,default,do,else,enum,sizeof,' +
    ...
    if (variables.length) {
        ...
        */
        ...
        for (var i = 0; i < variables.length; i++) {
            ...
            var varName = Blockly.Pawn.variableDB_.getName(variables[i].name, Blockly.Variables.NAME_TYPE);
            Blockly.Pawn.definitions_['variables'].push(varName);
...
Blockly.Pawn.finish = function(code) {
    ...
    for (var name in Blockly.Pawn.definitions_) {
        ...
        if(name == 'variables'){
            for(var variable in Blockly.Pawn.definitions_[name]){
                var varName = Blockly.Pawn.definitions_[name][variable];

                if(Blockly.Pawn.argumentDB_ && Blockly.Pawn.argumentDB_[varName] && Blockly.Pawn.argumentDB_[varName].count > 0){


                    definitions.push('new ' +( Blockly.Pawn.argumentDB_[varName].isFloat ? 'Float:' : '') + varName +
                        ( Blockly.Pawn.argumentDB_[varName].isPacked ? '{': '[') +
                        Blockly.Pawn.argumentDB_[varName].count +
                        ( Blockly.Pawn.argumentDB_[varName].isPacked ? '}': ']') +
                        ';');

                }else{
                    definitions.push('new ' + (Blockly.Pawn.argumentDB_ && Blockly.Pawn.argumentDB_[varName] && Blockly.Pawn.argumentDB_[varName].isFloat  ? 'Float:' : '') + varName + ';');
                }
            }
        }
    }
    ...
    delete Blockly.Pawn.argumentDB_;
    Blockly.Pawn.variableDB_.reset();
